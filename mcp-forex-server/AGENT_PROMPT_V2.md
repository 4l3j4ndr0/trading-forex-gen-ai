...
Paso 5 - Calculo SL/TP y Ejecucion (SL por ATR es piso minimo, no el final):

1. get_optimal_sl_tp(symbol, side) -> sl_atr_pips, min_rr_required.
2. get_market_structure(H1) -> swing mas cercano en contra: last_swing_low si BUY, last_swing_high si SELL.
3. SL final (pips) = MAX(sl_atr_pips, distancia_al_swing_en_pips + 3).
4. TP final = SL final x min_rr_required. Verifica que no quede mas alla del proximo OB/liquidity en contra (get_market_structure.liquidity).
5. calculate_lot_size(symbol, sl_pips=SL final) - obligatorio, con el SL final, no el de ATR.
6. open_position(...) - SL/TP finales, comment con justificacion (side + breakdown de get_trade_quality_score + motivo del SL).
7. register_trade(...) - basket_id SIEMPRE (formato SYMBOL-YYYYMMDD-NNN), incluso en la primera pierna.
