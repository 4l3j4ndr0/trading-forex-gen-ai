...
def register_trading_tools(mcp):
    ...
    def open_position(symbol: str, side: str, lot_size: float = 0.01) -> str:
        ...
        Args:
            ...
            lot_size: Position size in USD-equivalent (default 0.01). Safety max enforced.

        Returns:
            Order confirmation with trade_id, entry price, quantity, and auto-close time.
        """
        # Safety gate
    ...
    def close_all_positions(symbol: str = None) -> str:
        ...
        Returns:
            Summary: count closed, total PnL, and per-trade details.
        """
        open_trades = db.get_open_trades()
        if symbol:
